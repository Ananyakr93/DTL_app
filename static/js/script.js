...
function updateUI(data) {
    ...
    document.getElementById("aqiStatus").innerText = data.current.aqi_status || "Unknown";

    // Update Pollutants with "N/A" fallback
    const setVal = (id, val) => {
        const el = document.getElementById(id);
        if (el) el.innerText = (val !== null && val !== undefined) ? val : "N/A";
    };

    setVal("pm25", data.current.pm2_5);
    setVal("pm10", data.current.pm10);
    setVal("no2", data.current.no2);
    setVal("so2", data.current.so2);
    setVal("o3", data.current.o3);
    setVal("co", data.current.co);

    // Update Source Badge
    const sourceEl = document.getElementById("dataSource");
    if (sourceEl) {
        const sourceName = data.current.aqi_source || data.current.source || "Unknown";
        sourceEl.innerText = sourceName;

        // Add specific class based on source for styling
        const badge = document.getElementById("dataSourceBadge");
        if (badge) {
            badge.className = "badge"; // reset
            if (sourceName.toLowerCase().includes("cpcb")) {
                badge.classList.add("badge-official");
                badge.style.backgroundColor = "#e8f5e9";
                badge.style.color = "#1b5e20";
                badge.style.border = "1px solid #c8e6c9";
            } else if (sourceName.toLowerCase().includes("aqicn")) {
    ...
    const locationName = data.current.station || data.city;
    document.getElementById("updateTime").innerText = `📍 ${locationName} | Live Update: ${now}`;
}
...
function startAutoRefresh() {
    // Start countdown timer which handles the refresh
    startCountdown();
}
